...
class DetailLaporanKepolisianPage extends StatefulWidget {
  ...
  State<DetailLaporanKepolisianPage> createState() =>
      _DetailLaporanKepolisianPageState();
}

class _DetailLaporanKepolisianPageState
    extends State<DetailLaporanKepolisianPage> {
  bool _loading = true;
  bool _acting = false;
  ...
  Future<void> _load() async {
    ...
    try {
      final d = await PoliceReportService.fetchMineDetail(widget.reportId);
      if (!mounted) return;
      setState(() => _data = d);
    } catch (e) {
      _snack("❌ $e");
  ...
  Future<void> _cancel() async {
    ...
    final assigned = d?['assigned_officer_user_id'];

    final canCancel = status == 'pending' && assigned == null;
    if (!canCancel) {
      _snack("⚠️ Laporan tidak bisa dibatalkan.");
      return;
    }
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          "Pengajuan laporan akan dibatalkan dan tidak bisa diproses oleh officer.\n\nLanjutkan?",
        ),
        actions: [
          TextButton(
    ...
    if (ok != true) return;
    if (!canCancel) {
      _snack("⚠️ Laporan sudah diproses, tidak bisa dibatalkan.");
      return;
    }

    setState(() => _acting = true);
    try {
      await PoliceReportService.cancelMine(widget.reportId);
      await _load();
      _snack("✅ Pengajuan dibatalkan");
    } catch (e) {
      _snack("❌ $e");
    } finally {
  ...
  Widget build(BuildContext context) {
    final d = _data;
    final statusRaw = (d?['status'] ?? 'pending').toString();
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                children: [
                  ...
                  else if (statusRaw == 'pending' && assignedOfficer != null)
                    const Padding(
                      padding: EdgeInsets.only(top: 6),
                      child: Text(
                        "⚠️ Laporan sudah diambil officer, tidak bisa dibatalkan.",
