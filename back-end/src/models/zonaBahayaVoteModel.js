...
const getVoteSummary = async (idZona) => {
  ...
  const row = rows[0] || {};
  const total_setuju = Number(row.total_setuju || 0);
  const total_tidak = Number(row.total_tidak || 0);
  const total_vote = Number(row.total_vote || 0);

  const persen_setuju =
    total_vote === 0 ? 0 : Math.round((total_setuju * 100) / total_vote);

  const persen_tidak_setuju =
    total_vote === 0 ? 0 : Math.round((total_tidak * 100) / total_vote);
...
// Rekap vote berdasarkan ID laporan
const getVoteSummaryByLaporan = async (idLaporan) => {
  const [rows] = await db.execute(
    `
    SELECT
